class DashboardController < ApplicationController
  ...
  def setup_learner_dashboard
    @available_journals = Journal.published.where(visibility: ['public', 'unlisted']).limit(5)
    @recent_submissions = current_user.journal_submissions.includes(:journal).order(updated_at: :desc).limit(5)
    @new_journals_this_week = Journal.where(created_at: 1.week.ago..Time.current).count
  end

  def setup_instructor_dashboard
    @my_journals = current_user.journals.includes(:theme, :questions).order(updated_at: :desc).limit(5)
    @recent_submissions = JournalSubmission.joins(:journal)
                                          .where(journals: { user: current_user })
                                          .includes(:user, :journal)
  ...
  def setup_admin_dashboard
    ...
    @recent_activity = JournalSubmission.includes(:user, :journal)
                                       .order(updated_at: :desc)
                                       .limit(10)
  end
end
